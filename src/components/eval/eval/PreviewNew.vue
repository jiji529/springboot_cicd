<template>
	<div class="preview_wrap do_eval_preview" id="evalPreviewWrap">
		<div class="cont_tit">
		...
		<div class="preview " ref="preview" id="previewDiv"
		...
		<div v-if="isPaperOrOnline === 'aef' && isShowImage === true" class="aefArticle" :class="{blind : !listLayout2}">
			...
			<img class="aefArticleImage" :src="aefImage"/><br>
			<h1><span style="width:30px;"> &nbsp;</span></h1>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['selectedArticle', 'articleList', 'expandArticleList', 'showDoEvalMulti', 'multiEvalArticleList', 'newsIdList','multiNewsIdList', 'pid','hideAndShowArticleList', 'listLayout0', 'listLayout1', 'listLayout2']),
			...
			isPaperOrOnline() {
				if(this.selectedArticle === undefined || this.selectedArticle === '') {
					return 'no'
				} else if(Object.keys(this.selectedArticle).length > 10){
					if(this.selectedArticle.part_name === '') {
						return 'paper'
					} else if(this.selectedArticle.part_name === 'ht5'){
						return 'ht5'
					} else if(this.selectedArticle.part_name === 'aef'){
						return 'aef'
					}
				}
			},
		...
		watch : {
			selectedArticle: function () {
				if (this.selectedArticle.constructor.name === 'Object') {
					this.articleChange();
				}
			},
		...
		methods: {
			...
			async articleChange() {
				...
				if (this.isPaperOrOnline === 'no') {
					...
					await this.$axios.post(ssdo + '/getHtmlArticle.php',param)
						.then((res) => {
							...
							if (data.success === false) {
								...
								this.htmlContents = data.replace('onError="this.src', 'onError="this.onError=null;this.src');
								if(article.keywords !== null && article.keywords !== '')  {
									highLightWord = article.keywords.split(' ');
									for(const word in highLightWord) {
										this.htmlContents = this.replaceAll(this.htmlContents, highLightWord[word], '<span style="background-color: rgba(255,0,0,0.3)">'+highLightWord[word]+'</span>');
									}
								}
							}
					...
					await this.$axios.post(ssdo + '/getAefImage.php',param)
						.then(res =>  { let data = res.data;    this.aefImage = data; }).catch(e=>console.log(e));
					...
					// let newsTime = article.article_date.split(":");
					// newsTime.pop();
			...
			preArticle() {
				this.$eventBus.$emit("toDoEvalFromPreview" , "pre" );
			},
			nextArticle() {
				this.$eventBus.$emit("toDoEvalFromPreview" , "next");
			},
			setZero() {
			...
			setSize(){
				if(this.isPaperOrOnline === 'paper') {
					...
					this.frameHeight = this.$refs.preview.clientHeight - 20 -34 ; // pading: 20px, 스크롤바 때문인지 왜인지 36px 더 작게 나옴(17x2(scrollbar)+2(border))
					this.frameWidth = this.$refs.preview.clientWidth - 22; //padding : 20px , border: 2 px, scrollBar : 17px
				}
			},
			zoomIn(){
				if(this.isPaperOrOnline === 'paper') {
			...
			getMiniMapLine() {
				if(Object.keys(this.selectedArticle).length>3 ) {
					if(this.isPaperOrOnline === 'paper') {
						this.coordinateInfo = this.selectedArticle.coordinate;
						if (this.coordinateInfo === this.selectedArticle.coordinate) {
							let commaIndex = '';
							let pipeIndex = '';
			...
			moveDrag (e){
				if(this.isDraggable){
					...
					const frame = document.getElementById('previewDiv');
			...
			print() {
				if(this.isPaperOrOnline === 'paper') {
					...
					print.document.open();
					print.document.write(paperPrint(this.filePath));
				...
				function paperPrint(source){
					return  "<!DOCTYPE html><html><head><script>function step1(){\n" +
						...
						"</scri" + "pt>\n"+
						"<style>@page{size:A4; margin:10mm auto;} @media print{ html,body{width:210mm;height:277mm;} "+
			...
			highLightCheck(clientW, clientH){
				if(this.isPaperOrOnline === 'paper' && this.selectedArticle.highLight !== '') {
					...
					if(highLight !== '') {
						...
						for (const wordA in coordinateSplit) {
							if (coordinateSplit[wordA] !== null) {
								...
								if (coordinateSplit[wordA][2] <= this.naturalWidth &&
									coordinateSplit[wordA][3] <= this.naturalHeight &&
									coordinateSplit[wordA][4] <= this.naturalWidth &&
									coordinateSplit[wordA][3] <= this.naturalHeight) {
						...
						for (const key in relativeCoordinate) {
							if (relativeCoordinate[key] !== null) {
