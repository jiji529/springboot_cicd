  ...
  <div class="sch_area" v-if="getConfigEval" v-show=" this.getStatSetting.searchOpen">
    <div class="sch_cont">
      ...
      <dl class="sch_element">
        ...
        <dd>
          ...
          <input type="radio" id="sch_key_and" name="src_key" :value="1" v-model="getStatSetting.keywordOption">
          <label for="sch_key_and"><span></span>AND</label>
          <input type="radio" id="sch_key_or" name="src_key" :value="2" v-model="getStatSetting.keywordOption">
      ...
      <dl class="sch_element">
        ...
        <dd class="border keyword">
          ...
          <input type="radio" id="sch_exp_and" name="sch_exp_and" :value="1" v-model="getStatSetting.notKeywordOption">
          <label for="sch_exp_and"><span></span>AND</label>
          <input type="radio" id="sch_exp_or" name="sch_key_or" :value="2" v-model="getStatSetting.notKeywordOption">
      ...
      <dl class="sch_element" v-if="getStatSetting.reportInTabView">
        ...
        <div id="customReport">
            <section>
              <span v-if="getStatSetting.report" :title="getStatSetting.report.title">{{getStatSetting.report.title}}</span>
            </section>
          <a class="ext_btn" @click="newModal('report')"></a>
        </div>
      </dl>
      ...
      <dl class="sch_element" v-if="getStatSetting.crossInTabView">
        ...
        <div id="templateOption">
            <section>
              <template v-for="(tem, ti) of getStatSetting.templInfoSettings"> 
                <span v-if="tem.name != 'Text'" :key="'template-'+ti" :title="tem.optionName.map(e => e.name).join(' - ')">
                  {{ti+1}}.{{tem.name}}&nbsp;-&nbsp;{{ tem.optionName.map(e => e.name).join(' - ') }}
                  <!-- <template v-for="t in tem.optionName">&nbsp;-&nbsp;{{t.name}}</template> -->
                </span>
                <span v-else :title="tem.value.text" :key="'template-'+ti">
                  {{ti+1}}.{{tem.name}}&nbsp;-&nbsp;{{tem.value.text}}
                </span>
              </template>
            </section>
          <a class="ext_btn" @click="newModal('template')"></a>
        </div>
      </dl>
    ...
    <div class="filter0">
      <div class="ft_title">
        ...
        <span class="option">
          <input type="radio" id="sch_stat_auto_and" name="src_stat_auto" value="1" v-model.number="getStatSetting.evaluation0Options" @change="selectEval0Type">
            <label for="sch_stat_auto_and">AND</label>
          <input type="radio" id="sch_stat_auto_or" name="src_stat_auto" value="2" v-model.number="getStatSetting.evaluation0Options" @change="selectEval0Type">
    ...
    <div class="filter2">
      <div class="ft_title">
        ...
        <span class="option">
          <input type="radio" id="sch_stat_and" name="src_stat" value="1"
                 v-model.number="getStatSetting.evaluation2Options"
                 @change="selectEval2Type">
            <label for="sch_stat_and">AND</label>
          <input type="radio" id="sch_stat_or" name="src_stat" value="2"
                 v-model.number="getStatSetting.evaluation2Options"
                 @change="selectEval2Type">
...
<script>
  ...
  export default {
    name: 'StatSearch',
    components: {Datepicker, MediaSelection},
    ...
    methods: {
      ...
      newModal(mode) {
        if (mode == 'report') this.$emit('newModal', true, 1);
        else this.$emit('newModal', true, 2);
      },

      goSearch() { this.$emit('input', true); },
       ...
       */
      selectYear() {
        if (this.getStatSetting.selectYear) {
          let sMonth = 0;
          let eMonth = 12;
          if (this.getStatSetting.dateType === '1') {
            sMonth = Number(this.getStatSetting.selectMonth) - 1;
            eMonth = Number(this.getStatSetting.selectMonth);
          }
          this.getStatSetting.dateRange.startDate = new Date(this.getStatSetting.selectYear, sMonth, 1);
          this.getStatSetting.dateRange.endDate = new Date(this.getStatSetting.selectYear, eMonth, 0);
        }
      },
      /**
       * 연간 월간 기간 선택
      ...
      reset() {
        this.getStatSetting.searchRange = Object.keys(this.getNewsGroupInfo);
        this.getStatSetting.groupIsCategory=true;
        this.selectMediaAll({target: {checked: true}});
        this.getStatSetting.mediaCategoryList = this.categoryList;
        this.getStatSetting.mediaTypeList = this.typeList;
        this.getStatSetting.selectYear = new Date().getFullYear();
        this.getStatSetting.dateType = 1;
        this.searchRangeIsAll();
        this.getStatSetting.searchOption = 1;
        this.getStatSetting.keyword='';
        this.getStatSetting.keywordOption = 1;
        this.getStatSetting.notKeyword='';
        this.getStatSetting.notKeywordOption=1;
        this.getStatSetting.searchCriteria.includeHideArticles=true;
        this.getStatSetting.searchCriteria.includingNot=true;
        this.getStatSetting.searchCriteria.includingPrivate=true;
        this.getStatSetting.evaluation0Options = 2;
        this.getStatSetting.evaluation2Options = 2;
        this.getStatSetting.evaluationItem0 = [];
        this.getStatSetting.evaluationItem1 = [];
        this.getStatSetting.evaluationItem2 = [];
        this.getStatSetting.crossCalcValue = 0;
        this.getStatSetting.numberScale= 1;
      },
      ...
      async statSearch() {
        this.funcSelItems(); // checkBox 확인 함수
        let result = [];
        ...
        } else if (this.tableOrChart.length === 0) {
          alert('테이블 및 차트틑 추가해 주세요!!');
        } else {
          let val = this.getStatSetting.crossBarChartWidth
          if( val > 1 || val < 0.1) {
            alert('BarChart 폭 사이즈는 0~1 사이 값으로 넣어주세요!');
            this.getStatSetting.crossBarChartWidth = 1;
          }
          this.$emit('search', this.tableOrChart, this.ready);
          this.folding();
        }
      },

      folding() {
        this.tableOrChart.forEach(e => {
          e.show = false;
        });
        document.querySelectorAll('.optionList').forEach(e => {
          e.style.height = '50px';
        });
      },
